...
""" INIS specific authors printing. """
    ...
    for author in authors:
        ...
        if 'a' in author:
            if highlight == 'yes':
                ...
                author['a'] = bibformat_utils.highlight(generate_name(author),
                                                        bfo.search_pattern)

            if print_links.lower() == "yes":
                if link_author_pages == "no":
                    author['a'] = '<a itemprop="creator" href="' + \
                                  '/search?f=author&amp;p=' + quote(generate_name(author)) + \
                                  '&amp;ln=' + str(bfo.lang) + \
                                  '" ><span itemscope itemtype="http://schema.org/Person"><span itemprop="name">' + \
                        escape(
                            generate_name(author)) + '</span></span></a>'
                else:
                    author['a'] = '<a itemprop="creator" rel="author" href="' + \
                                  '/author/' + quote(generate_name(author)) + \
                                  '?recid=' + bibrec_id + \
                                  '&ln=' + str(bfo.lang) + \
                                  '">' + escape(generate_name(author)) + '</a>'

        if print_affiliations == "yes":
            author['u'] = affiliation_prefix + '<span itemprop="affiliation">' + generate_affiliation(author) + \
...
def generate_affiliation(author):
    template = "%(aff)s%(city)s%(country)s %(email)s"

    return template % {'aff': author['n'] if 'n' in author else '',
                       'city': ", " + author['k'] if 'k' in author else '',
                       'country': " (" + author['l'] + ')' if 'l' in author else '',
                       'email': ', ' + author['e'] if 'e' in author else '',
                       }


def generate_name(author):
    template = "%(firstname)s %(familyname)s"

    return template % {'firstname': author['b'],
                       'familyname': author['a'],
                       }
