...
def format_element(bfo, prefix_en, prefix_fr, suffix_en, suffix_fr, limit, max_chars,
    ...
    abstract_en = bfo.fields('860__a', escape=escape_mode_int)
    abstract_en.extend(bfo.fields('860__b', escape=escape_mode_int))
    abstract_en = separator_en.join(abstract_en)

    abstract_fr = bfo.fields('860__a', escape=escape_mode_int)
    abstract_fr.extend(bfo.fields('860__b', escape=escape_mode_int))
    ...
    if len(abstract_en) > 0 and 'en' in languages:
        ...
        if limit != "" and limit.isdigit():
            # Split around DOTSPACE so that we don't split html liks
