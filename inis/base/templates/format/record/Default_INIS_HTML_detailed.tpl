...
<tr>
{% set authors = bfe_authors(bfo, relator_code_pattern="$", limit="25", interactive="yes", print_affiliations="no") %}
{% if authors %}
...
{% endif %}
{{ bfe_abstract(bfo, prefix='<tr><th><i class="fa fa-pencil fa-fw"></i> Abstract</th><td>',suffix='</td></tr>' ) }}
{{ bfe_descriptors(bfo, prefix='<tr><th><i class="fa fa-pencil fa-fw"></i> Descriptors</th><td>',suffix='</td></tr>' ) }}
<tr>
    <th><i class="fa fa-calendar fa-fw"></i> Published</th>
    <td>{{ bfe_publisher(bfo) }}{{ bfe_publication_date(bfo, prefix="</br>") }}</td>
</tr>
{{ bfe_notes(bfo, note_prefix="", prefix='<tr><th><i class="fa fa-pencil fa-fw"></i> Notes</th><td>',suffix='</td></tr>' ) }}
